...
@snakes.plugins.plugin(
    "snakes.nets", depends=["snakes.plugins.let", "snakes.plugins.status"])
def extend(module):
    snk = module
            ...
            if self.pids.spawned:
                ...
                for parent, children in self.pids.spawned.items():
                    ...
                    for n, child in enumerate(children):
                        assign.append(
                            "%s=%s.next(%s+%s)" % (child, parent, n, pidcount))
                if guard is None:
    ...
    class PetriNet(snk.PetriNet):
        def __init__(self, name, **args):
            ...
            self.add_place(
                snk.Place(nextpids, [], tNextPid, status=snk.buffer(nextpids)))
        ...
        def add_place(self, place, **args):
            if place.name == self.nextpids:
                raise ConstraintError("reserved place name %r" % self.nextpids)
        ...
        def add_transition(self, trans, **args):
            ...
            for parent, children in trans.pids.spawned.items():
                ...
                cons[parent] = snk.Tuple(
                    [snk.Variable(parent),
                     snk.Variable(pidcount)])
                prod[parent] = snk.Tuple([
                    snk.Variable(parent),
                    snk.Expression("%s+%s" % (pidcount, len(children)))
                ])
                for num, child in enumerate(children):
                    prod[child] = snk.Tuple(
                        [snk.Variable(child),
                         snk.Value(0)])
