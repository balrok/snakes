...
class Graph(Cluster):
    ...
    def _dot(self):
        ...
        lines = [
            "subgraph %s {" % self,
            self._dot_attr(self.attr, "graph"), body, "}"
        ]
    ...
    def layout(self, engine="dot", debug=False):
        ...
        if PY3:
            out, err = dot.communicate(
                bytes(self.dot(), snakes.defaultencoding))
            out = out.decode(snakes.defaultencoding)
        else:
            out, err = dot.communicate(self.dot())
        if dot.returncode != 0:
            raise IOError(
                "%s exited with status %s" % (engine, dot.returncode))
...
@snakes.plugins.plugin(
    "snakes.nets", depends=["snakes.plugins.clusters", "snakes.plugins.pos"])
def extend(module):
    class PetriNet(module.PetriNet):
        ...
        def draw(self,
            ...
            g = self._copy(nodemap, self.clusters, cluster_attr, place_attr,
                           trans_attr)
            g.engine = engine
        ...
        def _copy(self, nodemap, sub, cluster_attr, place_attr, trans_attr):
            ...
            for child in sub.children():
                graph.add_child(
                    self._copy(nodemap, child, cluster_attr, place_attr,
                               trans_attr))
            return graph
