...
class DocExtract(object):
    ...
    def process(self):
        ...
        for dirpath, dirnames, filenames in os.walk(self.path):
            for name in sorted(filenames):
                ...
                try:
                    node = parse(
                        codecs.open(path, encoding=self.inputenc).read())
            ...
            if len(items) >= 2 and items[0].lower() == "apidoc":
                if len(items) == 2 and items[1].lower() in ("skip", "stop"):
                    ...
                    try:
                        ...
                        err("invalid directive %r (line %s)" % (dirline,
                                                                num + 1))
                    self.write_include(path, **args)
                else:
                    err("unknown directive %r (line %s)" % (items[1], num + 1))
                    return None
    ...
    def write_function(self, node):
        ...
        if self.classname:
            self.writeline(
                "#### Method `%s.%s` ####" % (self.classname, node.name))
        else:
    ...
    def write_epydoc(self, doc):
        ...
        for item in doc.lstrip("@").split("\n@"):
            ...
            if isinstance(info.get(tag, None), list):
                ...
                info[tag] = text
        if any(
                k in info for k in ("author", "organization", "copyright",
                                    "license", "contact")):
            self.newline()
            ...
            for tag in (
                ...
                if tag in info:
                    ...
                    self.writetext(
                        '<span class="api-title">%s:</span> %s' %
                        (tag.capitalize(), self.md(info[tag])),
                        subsequent_indent="  ")
                    self.writeline('</li>')
            self.writeline('</ul>')
        if any(info[k]
               for k in ("todo", "note", "attention", "bug", "warning")):
            self.newline()
        ...
        if (any(info[k] for k in ("param", "type", "keyword"))
            ...
            for arg in self.args:
                if arg in info["param"]:
                    self.writelist("`%s %s`: %s" % (info["type"].get(
                        arg, "object").strip("`"), arg, info["param"][arg]))
                else:
                    self.writelist("`%s %s`" % (info["type"].get(
                        arg, "object").strip("`"), arg))
            for kw, text in sorted(info["keyword"].items()):
                self.writelist("keyword `%s`: %s" % (kw, text))
            if any(k in info for k in ("return", "rtype")):
                if "return" in info:
                    self.writelist("`return %s`: %s" % (info.get(
                        "rtype", "object").strip("`"), info["return"]))
                else:
                    self.writelist("`return %s`" % (info.get(
                        "rtype", "object").strip("`")))
        if info["raise"]:
            self.newline()
