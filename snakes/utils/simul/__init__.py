...
class BaseSimulator(object):
    ...
    def getstate(self, state):
        ...
        places = [
            ...
            for place in sorted(
                self.states.net.place(), key=operator.attrgetter("name"))
        ]
        modes = [{
            "state":
            state,
            ...
            i,
            "html":
            "%s : %s" % (H.span(trans.name, class_="trans"),
...
class BaseHTTPSimulator(Node):
    def __init__(self, net=None, port=8000, respatt=[], simulator=None):
        ...
        for cls in reversed(inspect.getmro(self.__class__)[:-2]):
            ...
            for pattern in respatt + ["resources/*/*.*", "resources/*.*"]:
                for res in glob.glob(os.path.join(path, pattern)):
                    if os.path.isfile(res):
                        with open(res) as infile:
                            self.res[res[len(path + "resources/") +
                                         1:]] = infile.read()
    ...
    def init(self, state=-1):
        ret = {"ui": self.init_ui(), "help": self.simul.init_help()}
