...
class Builder(object):
    ...
    def _build(self, node, ctx):
        if isinstance(node, ast.atom):
            ...
            node.atomic = (isinstance(node.op,
                                      (ast.boolop, ast.Imply, ast.Iff))
                           and node.left.atomic and node.right.atomic)
        elif isinstance(node, ast.CtlUnary):
            node.child = self._build(node.child, ctx)
            node.atomic = (isinstance(node.op, ast.Not) and node.child.atomic)
        else:
    ...
    def _build_place(self, param, ctx):
        if isinstance(param, ast.Parameter):
            if param.name not in ctx:
                raise SpecError(param,
                                "place %r should be instantiated" % param.name)
    ...
    def _build_Instance_Property(self, node, prop, ctx):
        ...
        for param in prop.params:
            if param.name in bound:
                raise SpecError(node, "argument %r already bound" % param.name)
            ...
            if param.type == "place":
                if not isinstance(arg, ast.Place):
                    raise SpecError(node, "expected place for %r" % param.name)
                arg.name = param.name
            ctx[param.name] = arg
        if args:
            raise SpecError(
                node,
                "too many arguments (%s)" % ", ".join(repr(a) for a in args))
    ...
    def _build_Instance_Atom(self, node, atom, ctx):
        ...
        for param in atom.params:
            ...
            if param.type == "place":
                ...
                arg = ast.Argument(name=param.name, value=arg, type=param.type)
            new.args.append(arg)
        if args:
            raise SpecError(
                node,
