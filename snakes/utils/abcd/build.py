...
class Builder(object):
    def __init__(self, snk, path=[], up=None):
        ...
        if up:
            ...
            self.globals = snk.Evaluator(
                dot=self.snk.dot, BlackToken=self.snk.BlackToken)
                    ...
                    for c in decl.capacity:
                        if c is None:
                            ...
                            try:
                                ...
                                self._raise(
                                    CompilationError,
                                    "could not evaluate %r, %s" % (unparse(c),
                                                                   err))
    ...
    def _build_True(self, node):
        ...
        e = self.snk.Place(
            "e", [], self.snk.tBlackToken, status=self.snk.entry)
        ...
        t.label(
            srctext=node.st.source(),
            srcloc=(node.st.srow, node.st.scol, node.st.erow, node.st.ecol),
            path=self.path)
    ...
    def build_SimpleAccess(self, node):
        ...
        b.label(
            ...
            srcloc=(decl.node.st.srow, decl.node.st.scol, decl.node.st.erow,
                    decl.node.st.ecol))
        net.add_place(b)
    ...
    def build_SwapAccess(self, node):
        ...
        net.add_transition(
            self.snk.Transition("t", status=self.snk.tick("action")))
        b = self.snk.Place(
            node.buffer, [], decl.type, status=self.snk.buffer(node.buffer))
        b.label(
            path=self.path,
            buffer=str(node.buffer),
            srctext=decl.node.st.source(),
    ...
    def build_TupleType(self, node):
        return self.snk.Collection(
            self.snk.Instance(tuple), (self.build(node.items)))

    def build_SetType(self, node):
        return self.snk.Set(self.build(node.items))

    def build_DictType(self, node):
        return self.snk.Mapping(
            keys=self.build(node.keys),
