...
class ABCDSimulator(BaseSimulator):
    def __init__(self, node, net, gv):
        ...
        for trans in net.transition():
            ...
            if nid in n2html.n2a:
                self.abcd[trans.name] = ", ".join(
                    "#" + i for i in n2html.n2a[nid])
    ...
    def getstate(self, state):
        ...
        for i, (trans, mode) in enumerate(marking.modes):
            modes[self.tree[trans.name]].append({
                "state": state,
                "mode": i,
                "html": str(mode)
            })
        return {
            ...
            "states": [{
                ...
                "class": "active"
            } for trans, mode in marking.modes] +
            [{
                "do": "addclass",
            ...
            } for trans, mode in marking.modes] + [
                {
                    ...
                    "text": "{}"
                } for place in self.places if place not in marking
            ] + [{
                "do": "settext",
                "select": "%s .content" % self.tree[place],
                "text": str(marking[place])
            } for place in marking if place in self.tree],
            "modes": [{
                "select": "%s + .modes" % trans,
                "items": items
