...
class Span(object):
    ...
    def __nonzero__(self):
        return bool(self.cls or self.id or self.net or self.abcd or self.tree)

    def span(self):
        attr = []
        for src, dst in (("cls", "class"), ("id", "id"), ("net", "data-net"),
                         ("abcd", "data-abcd"), ("tree", "data-tree")):
...
keywords = {
    "False", "True", "and", "as", "assert", "buffer", "const", "else", "enum",
    "for", "from", "if", "import", "in", "is", "lambda", "net", "not", "or",
    "symbol", "task", "typedef"
}
    ...
    def build(self, st):
        ...
        for line in output.getvalue().splitlines(True):
            if "#" in line:
                ...
                self.output.write(
                    "%s<span class=%r>%s</span>" %
                    (self.escape(left), "comment", self.escape("#" + right)))
            else:
...
class Net2HTML(object):
    def __init__(self, net, gv, abcd):
        ...
        for place in net.place():
            ...
            if place.status in (snk.entry, snk.internal, snk.exit):
                for char, trans in (
                    [("R", net.transition(t))
                     for t in place.pre] + [("L", net.transition(t))
                                            for t in place.post]):
                    span = abcd[trans]
    ...
    def addtree(self, weight, kind, node):
        ...
        aid = self.abcd[node]
        tid = aid.copy(
            id=("T%X" % self.count[aid.id]) + aid.id,
            tree=[],
            abcd=[aid.id],
            net=[nid])
        self.count[aid.id] += 1
