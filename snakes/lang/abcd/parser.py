...
class Translator(PyTranslator):
    ...
    def do_abcd_main(self, st, ctx):
        ...
        for i, child in enumerate(st):
            if child.symbol == "abcd_expr":
                expr = i
                break
        return self.ST.AbcdSpec(
            lineno=st.srow,
            col_offset=st.scol,
            context=[
                self.do(child) for child in st[:expr]
    ...
    def do_abcd_const(self, st, ctx):
        ...
        """
        return self.ST.AbcdConst(
            lineno=st.srow,
            col_offset=st.scol,
            name=st[1].text,
            value=self.do(st[3], ctx))
    ...
    def do_abcd_symbol(self, st, ctx):
        ...
        """
        return self.ST.AbcdSymbol(
            lineno=st.srow, col_offset=st.scol, symbols=self.do(st[1]))
    ...
    def do_abcd_action(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            return self.ST.AbcdAction(
                lineno=st.srow, col_offset=st.scol, accesses=[], guard=True)
        elif st[1].text == "False":
            return self.ST.AbcdAction(
                lineno=st.srow, col_offset=st.scol, accesses=[], guard=False)
        elif len(st) == 3:
            return self.ST.AbcdAction(
                lineno=st.srow,
                col_offset=st.scol,
    ...
    _arc = {"+": ast.Produce, "-": ast.Consume, "?": ast.Test, "<<": ast.Fill}
    ...
    def do_abcd_access(self, st, ctx):
        ...
        if st[1].text in ("+", "?", "-"):
            ...
            if atom is not None:
                ...
                args = self.ST.Tuple(
                    lineno=st.srow, col_offset=st.scol, elts=elts, ctx=ctx())
            else:
                args = self.ST.ListComp(
                    lineno=st.srow,
                    col_offset=st.scol,
                    elt=loop,
                    generators=elts)
            ...
            return self.ST.FlushAccess(
                lineno=st.srow,
                col_offset=st.scol,
                buffer=st[0].text,
                target=st[3].text)
        elif st[1].text == "<>":
            return self.ST.SwapAccess(
                lineno=st.srow,
                col_offset=st.scol,
                buffer=st[0].text,
                target=self.do(st[3]),
            ...
            if len(st) > 6:
                raise ParseError(
                    st.text, reason="too many arguments for %s" % st[2].text)
            ...
            raise ParseError(
                st[2].text,
                reason=("expected 'spawn', 'wait', "
                        "'suspend' or 'resume', but found '%s'") % st[2].text)
    ...
    def do_abcd_buffer(self, st, ctx):
        ...
        if len(st) == 6:  # no decorator, no array
            ...
            raise ParseError(st.text, reason="arrays not (yet) supported")
            ...
            if isinstance(tree, self.ST.Call) and tree.func.id == "capacity":
                ...
                for kw in tree.keywords:
                    if kw.arg == "min":
                        ...
                        raise ParseError(
                            child, reason="invalid parameter %r" % kw.arg)
                if min or max:
    ...
    def do_abcd_and_type(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            return self.ST.IntersectionType(
                lineno=st.srow,
                col_offset=st.scol,
                types=[self.do(child) for child in st[::2]])
    ...
    def do_abcd_base_type(self, st, ctx):
        ...
        """
        if len(st) == 1:
            return self.ST.NamedType(
                lineno=st.srow, col_offset=st.scol, name=st[0].text)
        elif len(st) == 3:
            return self.do(st[1])
            ...
            if len(st) > 4:
                raise ParseError(
                    st.text,
                    reason="too many arguments for %s type" % st[0].text)
            if st[0].text == "list":
            ...
            return tree(
                lineno=st.srow, col_offset=st.scol, items=self.do(st[2]))
        elif st[0].text == "dict":
            if len(st) > 6:
                raise ParseError(
                    st.text, reason="too many arguments for dict type")
            return self.ST.DictType(
                lineno=st.srow,
                col_offset=st.scol,
                keys=self.do(st[2]),
                values=self.do(st[4]))
        elif st[0].text == "enum":
            return self.ST.EnumType(
                lineno=st.srow,
                col_offset=st.scol,
