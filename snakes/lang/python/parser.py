...
class Translator(object):
            ...
            while len(values) > 1:
                ...
                values.insert(
                    ...
                    self.ST.BinOp(
                        ...
                        col_offset=st.scol,
                        left=left,
                        op=operator(lineno=node.srow, col_offset=node.scol),
                        right=right))
            return values[0]
    ...
    def _do_boolean(self, st, ctx):
        if len(st) == 1:
            ...
            return self.ST.BoolOp(
                lineno=st.srow,
                col_offset=st.scol,
                op=self._boolean[st[1].text](
                    lineno=st[1].srow, col_offset=st[1].scol),
                values=[self.do(child, ctx) for child in st[::2]])
    ...
    def do_simple_stmt(self, st, ctx):
        ...
        """
        return [
            self.do(child, ctx) for child in st[::2]
    ...
    def do_expr_stmt(self, st, ctx):
        ...
        if len(st) == 1:
            return self.ST.Expr(
                lineno=st.srow, col_offset=st.scol, value=self.do(st[0], ctx))
        elif st[1].symbol == "augassign":
            target = self.do(st[0], self.ST.Store)
            if isinstance(target, self.ST.Tuple):
                raise ParseError(
                    st[0].text,
                    reason="illegal expression for"
                    " augmented assignment")
            return self.ST.AugAssign(
                lineno=st.srow,
                col_offset=st.scol,
                target=target,
                op=self.do(st[1], ctx),
            ...
            return self.ST.Assign(
                lineno=st.srow,
                col_offset=st.scol,
                targets=[self.do(child, ast.Store) for child in st[:-1:2]],
    ...
    def do_augassign(self, st, ctx):
        ...
        """
        return self._binary[st[0].text[:-1]](
    ...
    def do_del_stmt(self, st, ctx):
        ...
        if isinstance(targets, self.ST.Tuple):
            ...
            targets = [targets]
        return self.ST.Delete(
            lineno=st.srow, col_offset=st.scol, targets=targets)
    ...
    def do_return_stmt(self, st, ctx):
        ...
        """
        if len(st) == 1:
            return self.ST.Return(
                lineno=st.srow, col_offset=st.scol, value=None)

        else:
            return self.ST.Return(
                lineno=st.srow, col_offset=st.scol, value=self.do(st[1], ctx))
    ...
    def do_raise_stmt(self, st, ctx):
        ...
        count = len(st)
        if count == 1:
            return self.ST.Raise(
                lineno=st.srow, col_offset=st.scol, exc=None, cause=None)
        elif count == 2:
            return self.ST.Raise(
                lineno=st.srow,
                col_offset=st.scol,
    ...
    def do_import_name(self, st, ctx):
        ...
        "Module(body=[Import(names=[alias(name='foo', asname=None), alias(name='bar', asname=None), alias(name='egg', asname='spam')])])"
        """
        return self.ST.Import(
            lineno=st.srow, col_offset=st.scol, names=self.do(st[1], ctx))
    ...
    def do_except_clause(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            return self.do(st[1], ctx), self.ST.Name(
                lineno=st[3].srow,
    ...
    def do_lambdef(self, st, ctx):
        ...
        """
        if len(st) == 3:
            return self.ST.Lambda(
                lineno=st.srow,
                col_offset=st.scol,
                args=self.ST.arguments(
                    lineno=st.srow,
                    col_offset=st.scol,
                    args=[],
                    vararg=None,
    ...
    def do_comparison(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            return self.ST.Compare(
                lineno=st.srow,
                col_offset=st.scol,
                left=self.do(st[0], ctx),
                ops=[self.do(child, ctx) for child in st[1::2]],
                comparators=[self.do(child, ctx) for child in st[2::2]])
    ...
    def do_factor(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            if (isinstance(tree.op, self.ST.USub)
                ...
                tree = self.ST.Num(
                    lineno=st.srow, col_offset=st.scol, n=-tree.operand.n)
            return tree
    ...
    def do_power(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            if power:
                return self.ST.BinOp(
                    lineno=st.srow,
                    col_offset=st.scol,
                    left=left,
                    op=self.ST.Pow(lineno=st[-2].srow, col_offset=st[-2].scol),
                    right=power)
            else:
                return left
    ...
    def do_atom(self, st, ctx):
        ...
        if kind == self.NUMBER:
            ...
            return self.ST.Name(
                lineno=st.srow, col_offset=st.scol, id=text, ctx=ctx())
        elif kind == self.STRING:
            return self.ST.Str(
                lineno=st.srow,
                col_offset=st.scol,
                s="".join(self.ST.literal_eval(child.text) for child in st))
        elif text == "...":
            return self.ST.Ellipsis(lineno=st.srow, col_offset=st.scol)
        elif text == "[":
            if len(st) == 2:
                return self.ST.List(
                    lineno=st.srow, col_offset=st.scol, elts=[], ctx=ctx())
            else:
            ...
            if len(st) == 2:
                return self.ST.Dict(
                    lineno=st.srow, col_offset=st.scol, keys=[], values=[])
            else:
                return self.do(st[1], ctx)
    ...
    def do_sliceop(self, st, ctx):
        ...
        if len(st) == 1:
            return self.ST.Name(
                lineno=st.srow, col_offset=st.scol, id="None", ctx=ctx())
        else:
            return self.do(st[1], ctx)
    ...
    def do_testlist(self, st, ctx):
        ...
        if len(lst) == 1:
            ...
            return self.ST.Tuple(
                lineno=st.srow, col_offset=st.scol, elts=lst, ctx=ctx())
    ...
    def do_dictorsetmaker(self, st, ctx):
        ...
        if len(st) == 1:
            return self.ST.Set(
                lineno=st.srow, col_offset=st.scol, elts=[self.do(st[0], ctx)])
        elif st[1].text == ":":
            if len(st) < 4 or st[3].text == ",":
                return self.ST.Dict(
                    lineno=st.srow,
                    col_offset=st.scol,
                    keys=[self.do(child, ctx) for child in st[::4]],
                    values=[self.do(child, ctx) for child in st[2::4]])
            else:
                return self.ST.DictComp(
                    lineno=st.srow,
                    col_offset=st.scol,
            ...
            if loop is None:
                return self.ST.Set(
                    lineno=st.srow, col_offset=st.scol, elts=elts)
            else:
                return self.ST.SetComp(
                    lineno=st.srow,
                    col_offset=st.scol,
                    elt=loop,
                    generators=elts)
        ...
        while nodes:
            if nodes[0].text == "*":
                ...
                if isinstance(arg, self.ST.keyword):
                    if arg.arg in allkw:
                        raise ParseError(
                            nodes[0].text, reason="keyword argument repeated")
                    keywords.append(arg)
                    allkw.add(arg.arg)
    ...
    def do_argument(self, st, ctx):
        ...
        if len(st) == 1:
            ...
            if not isinstance(test, self.ST.Name):
                raise ParseError(
                    st[0].text, reason="keyword can't be an"
                    " expression")
            return self.ST.keyword(
                lineno=st.srow,
                col_offset=st.scol,
                arg=test.id,
                value=self.do(st[2], ctx))
            ...
            return self.ST.GeneratorExp(
                lineno=st.srow, col_offset=st.scol, elt=test, generators=comp)
    ...
    def parse(cls, expr, mode="exec", filename="<string>"):
        tree = cls(
            cls.parser.parseString(expr.strip() + "\n", filename=filename)).ast
        if mode == "exec":
            return tree
        elif mode == "eval":
    ...
    def parse(self, string, name="<string>"):
        ...
        for i, exple in enumerate(examples):
            if isinstance(exple, str):
                ...
                examples[i] = (
                    "skipping example for another language: %r" % exple.source)
                continue
            if rule is not None:
                source = exple.source.strip() + "\n"
                try:
                    tree = self.Translator.ParseTree(
                        self.Translator.parser.parseString(source))
                except:
